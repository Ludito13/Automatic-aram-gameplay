...
public class SpatialGrid : MonoBehaviour
{
    ...
    private Dictionary<Entity, Tuple<int, int>> lastPositions;
    //los "contenedores"
    private HashSet<Entity>[,] buckets;
    ...
    readonly public Entity[] Empty = new Entity[0];
    #endregion
    ...
    private void Awake()
    {
        lastPositions = new Dictionary<Entity, Tuple<int, int>>();
        buckets = new HashSet<Entity>[width, height];

        //creamos todos los hashsets
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
                buckets[i, j] = new HashSet<Entity>();
    ...
    public void RemoveFromTheList(Entity entity)
    {
    ...
    public IEnumerable<Entity> Query(Vector3 aabbFrom, Vector3 aabbTo, Func<Vector3, bool> filterByPosition)
    {
    ...
    void OnDestroy()
    {
        var ents = RecursiveWalker(transform).Select(x => x.GetComponent<Entity>()).Where(x => x != null);
        foreach (var e in ents)
            e.OnMove -= UpdateEntity;
    }
    ...
    private void OnDrawGizmos()
    {
        ...
        if (!activatedGrid)
        {
            IEnumerable<Entity> allElems = Enumerable.Empty<Entity>();
